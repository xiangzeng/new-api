...
- [ ] Phase 0 基线与证据契约：建立 worktree、任务档案和对标契约。
- [ ] Phase 1 核心模型与定价解析器：三库兼容模型、四级优先级、延迟涨价、乐观锁。
...
- 做了：确认 New API 仓库基线干净；创建 `feature/reseller-center` 同级 worktree；建立任务档案和行为契约。
- 基线：`353b702c2`。
- 下一步：实现 Phase 1 的核心模型、迁移注册、纯定价解析器和单元测试。
